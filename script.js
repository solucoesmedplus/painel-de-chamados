...
const CONFIG = {
  SCRIPT_URL: "https://script.google.com/macros/s/AKfycbwUrdNYyepwzpxKZDb06zFR--1U4JQVwQ2idEdWQ1TruoKi41xTJiNTS4_bX-zgW6yL/exec",
  SPREADSHEET_ID: "1a9_QQPxp56JcmWyDfQoLH9RuQxfTLbHPimfjImoUI8A"
};

/*
  Ao gerar um novo chamado, o sistema copia para a área de transferência
  o resumo pronto para colar manualmente no WhatsApp.
*/
...
const CHAMADOS_CACHE_KEY = `medplus_chamados_cache_${CONFIG.SPREADSHEET_ID || "demo"}`;
const CHAMADOS_CACHE_MAX_AGE_MS = 10 * 60 * 1000;
...
function updateAppChromeColor() {
  const isLight = document.body.classList.contains("theme-light");
  const color = isLight ? "#eef3f6" : "#071a23";
...
function bindCnpjPhoneMasks() {
  $$('input').forEach((input) => {
    const key = `${input.id || ""} ${input.name || ""}`.toLowerCase();
    const isCnpjField = key.includes("cnpj");
    const isPhoneField = key.includes("telefone") || key.includes("phone");

    if (!isCnpjField && !isPhoneField) return;

    const applyMaskAndLengthValidation = () => {
      input.value = isCnpjField ? maskCNPJ(input.value) : maskPhone(input.value);

      if (input.id === "cnpj") validateCNPJField();
      if (input.id === "telefone") validatePhoneField();
    };
    ...
    input.addEventListener("input", applyMaskAndLengthValidation);
    input.addEventListener("blur", applyMaskAndLengthValidation);
  });
}
...
function initAberturaPage() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const equipamentosAbertura = coletarEquipamentosAbertura();
    if (!equipamentosAbertura.length) {
      toast("Informe pelo menos um equipamento com modelo, serial ou marca.", "error");
      return;
    }
    sincronizarEquipamentosAbertura(equipamentosAbertura);
    ...
    try {
      ...
      payload.equipamentos = equipamentosAbertura;
      payload.modeloEquipamento = formatEquipamentosForDisplay(equipamentosAbertura);
      payload.equipamentosJson = JSON.stringify(equipamentosAbertura);
      ...
      const chamadoSalvo = response.data || {
        ...payload,
        numeroChamado: numero
      };

      atualizarChamadoNoEstado(chamadoSalvo);

      if (!editando) {
        const copiado = await copiarResumoChamadoAreaTransferencia(chamadoSalvo);

        toast(
          copiado
            ? `Chamado ${numero || "gerado"} salvo com sucesso! Dados copiados para a área de transferência.`
            : `Chamado ${numero || "gerado"} salvo com sucesso! Não foi possível copiar automaticamente; copie manualmente pelos dados do chamado.`,
          copiado ? "success" : "error"
        );
      } else {
        toast(`Chamado ${numero} atualizado com sucesso!`, "success");
      }

      form.reset();
      sairModoEdicao();
      initMasksAndDefaults();
      resetAberturaEquipamentos();
...
async function copiarResumoChamadoAreaTransferencia(chamado) {
  const mensagem = montarMensagemResumoChamado(chamado);
  ...
  try {
    ...
    console.warn("Falha ao copiar com Clipboard API. Tentando método alternativo.", error);
  }

  return copiarTextoFallback(mensagem);
}
...
function lerChamadosDoCache() {
  try {
    const raw = localStorage.getItem(CHAMADOS_CACHE_KEY);
    if (!raw) return [];

    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed.data)) return [];

    const idade = Date.now() - Number(parsed.savedAt || 0);
    if (idade > CHAMADOS_CACHE_MAX_AGE_MS) {
      // Mesmo quando o cache está antigo, ele ainda ajuda a tela abrir rápido.
      // Logo depois o sistema busca a planilha e corrige a lista automaticamente.
      return parsed.data;
    }
...
function salvarChamadosNoCache(chamados) {
  try {
    if (!Array.isArray(chamados)) return;
    localStorage.setItem(CHAMADOS_CACHE_KEY, JSON.stringify({
      savedAt: Date.now(),
      data: chamados
    }));
  } catch (_) {
    // Cache local é apenas otimização. Se falhar, o sistema continua normal.
  }
}

function hidratarListaComCache(tipo = "painel") {
  const cached = lerChamadosDoCache();
  if (!cached.length) return false;

  if (tipo === "admin") {
    state.adminChamados = cached;
    renderAdminChamados();
    return true;
  }

  state.chamados = cached;
  aplicarFiltros();
  return true;
}

function removerChamadoDoEstado(numeroChamado) {
  if (!numeroChamado) return;

  state.chamados = state.chamados.filter((item) => item.numeroChamado !== numeroChamado);
  state.filtrados = state.filtrados.filter((item) => item.numeroChamado !== numeroChamado);
  state.adminChamados = state.adminChamados.filter((item) => item.numeroChamado !== numeroChamado);

  if (state.chamadoAtual?.numeroChamado === numeroChamado) {
    state.chamadoAtual = null;
  }

  const listaCache = state.chamados.length ? state.chamados : state.adminChamados;
  salvarChamadosNoCache(listaCache);

  if ($("#chamadosList")) aplicarFiltros();
  if ($("#adminChamadosTbody")) renderAdminChamados();
}

function montarMensagemResumoChamado(chamado) {
  ...
  return `⚠️ *_Painel de Chamados_* ⚠️
` +
    `======= *Informa* ======= 
...
function bindAdminChamados() {
  ...
  $("#adminReloadBtn")?.addEventListener("click", () => carregarAdminChamados({ useCache: false }));
  ...
  hidratarListaComCache("admin");
  window.setTimeout(() => carregarAdminChamados({ silent: true }), 350);
}

async function carregarAdminChamados(options = {}) {
  const { silent = false, useCache = true } = options;
  const btn = $("#adminReloadBtn");

  if (useCache && !state.adminChamados.length) {
    hidratarListaComCache("admin");
  }

  if (!silent) setLoading(btn, true, "Atualizando...");

  try {
    const response = await apiGet("listChamados");
    state.adminChamados = Array.isArray(response.data) ? response.data : [];
    salvarChamadosNoCache(state.adminChamados);
    renderAdminChamados();
    ...
    console.error(error);
    if (!silent) toast(error.message || "Erro ao carregar a área administrativa.", "error");
    if (!state.adminChamados.length) {
      state.adminChamados = [];
      renderAdminChamados();
    }
  } finally {
    if (!silent) setLoading(btn, false, "🔄 Atualizar lista");
  }
}
...
function editarChamadoAdmin(numeroChamado) {
  ...
  renderAberturaEquipamentos(chamado);
...
async function excluirChamadoAdmin(numeroChamado) {
  ...
  try {
    ...
    removerChamadoDoEstado(numeroChamado);
  } catch (error) {
    console.error(error);
...
function initAberturaEquipamentos() {
  const container = $("#equipamentosAberturaContainer");
  if (!container) return;

  const addBtn = $("#addEquipamentoAberturaBtn");
  if (addBtn && addBtn.dataset.boundEquipamentos !== "true") {
    addBtn.dataset.boundEquipamentos = "true";
    addBtn.addEventListener("click", () => addAberturaEquipamentoRow());
  }

  if (!$$(".equipamento-row", container).length) {
    addAberturaEquipamentoRow();
  }

  sincronizarEquipamentosAbertura();
}

function resetAberturaEquipamentos() {
  const container = $("#equipamentosAberturaContainer");
  if (!container) return;
  container.innerHTML = "";
  addAberturaEquipamentoRow();
...
function renderAberturaEquipamentos(chamado) {
  const container = $("#equipamentosAberturaContainer");
  if (!container) return;
  container.innerHTML = "";

  const equipamentos = parseEquipamentosFromChamado(chamado);
  (equipamentos.length ? equipamentos : [{}]).forEach((equipamento) => addAberturaEquipamentoRow(equipamento));
  sincronizarEquipamentosAbertura();
}

function addAberturaEquipamentoRow(value = {}) {
  const container = $("#equipamentosAberturaContainer");
  if (!container) return;
  const row = createEquipamentoRow(container, value, "abertura-equipamento-row");
  container.appendChild(row);
  sincronizarEquipamentosAbertura();
}

function sincronizarEquipamentosAbertura(equipamentos = coletarEquipamentosAbertura()) {
  const modeloInput = $("#modeloEquipamento");
  const jsonInput = $("#equipamentosJson");
  const resumo = formatEquipamentosForDisplay(equipamentos);

  if (modeloInput) modeloInput.value = resumo;
  if (jsonInput) jsonInput.value = JSON.stringify(equipamentos);
}

function coletarEquipamentosAbertura() {
  return coletarEquipamentosContainer("#equipamentosAberturaContainer");
}

function coletarOSEquipamentos() {
  return coletarEquipamentosContainer("#equipamentosContainer");
}

function coletarEquipamentosContainer(selector) {
  const container = $(selector);
  if (!container) return [];

  return $$(".equipamento-row", container)
    .map(extractEquipamentoFromRow)
    .filter(hasEquipamentoValue);
}

function createEquipamentoRow(container, value = {}, extraClass = "") {
  const equipamento = normalizeEquipamentoItem(value);
  const row = document.createElement("div");
  row.className = `equipamento-row ${extraClass}`.trim();
  row.innerHTML = `
    <input type="text" class="equipamento-input equipamento-modelo" placeholder="Modelo" value="${escapeAttr(equipamento.modelo)}" />
    <input type="text" class="equipamento-input equipamento-serial" placeholder="Serial" value="${escapeAttr(equipamento.serial)}" />
    <input type="text" class="equipamento-input equipamento-marca" placeholder="Marca" value="${escapeAttr(equipamento.marca)}" />
    <button class="remove-equip-btn no-print" type="button" title="Remover equipamento">×</button>
  `;

  $$("input", row).forEach((input) => {
    input.addEventListener("input", () => {
      syncPrintValue(input);
      if (container.id === "equipamentosAberturaContainer") sincronizarEquipamentosAbertura();
    });
    syncPrintValue(input);
  });

  row.querySelector("button")?.addEventListener("click", () => {
    const rows = $$(".equipamento-row", container);
    if (rows.length === 1) {
      $$("input", row).forEach((input) => {
        input.value = "";
        syncPrintValue(input);
...
function extractEquipamentoFromRow(row) {
  return normalizeEquipamentoItem({
    modelo: row.querySelector(".equipamento-modelo")?.value || "",
    serial: row.querySelector(".equipamento-serial")?.value || "",
    marca: row.querySelector(".equipamento-marca")?.value || ""
  });
}

function normalizeEquipamentoItem(value = {}) {
  if (typeof value === "string") {
    const texto = value.trim();
    return { modelo: texto, serial: "", marca: "" };
  }

  return {
    modelo: String(value.modelo || value.model || value.equipamento || value.nome || value.descricao || "").trim(),
    serial: String(value.serial || value.numeroSerie || value.serie || value.sn || "").trim(),
    marca: String(value.marca || value.fabricante || value.brand || "").trim()
  };
}

function hasEquipamentoValue(equipamento) {
  return Boolean(equipamento && (equipamento.modelo || equipamento.serial || equipamento.marca));
}

function parseEquipamentosFromChamado(chamado = {}) {
  const parsed = parseMaybeJSON(chamado.equipamentosJson || chamado.equipamentos || chamado.equipamentosJsonAbertura);

  if (Array.isArray(parsed) && parsed.length) {
    return parsed.map(normalizeEquipamentoItem).filter(hasEquipamentoValue);
  }

  if (Array.isArray(chamado.equipamentos) && chamado.equipamentos.length) {
    return chamado.equipamentos.map(normalizeEquipamentoItem).filter(hasEquipamentoValue);
  }

  const legado = String(chamado.modeloEquipamento || "").trim();
  if (!legado) return [];

  return legado
    .split(/\r?\n+/)
    .map(normalizeEquipamentoItem)
    .filter(hasEquipamentoValue);
}

function formatEquipamentoForDisplay(equipamento) {
  const item = normalizeEquipamentoItem(equipamento);
  const partes = [];
  if (item.modelo) partes.push(item.modelo);
  if (item.serial) partes.push(`Serial: ${item.serial}`);
  if (item.marca) partes.push(`Marca: ${item.marca}`);
  return partes.join(" | ");
}

function formatEquipamentosForDisplay(equipamentos = []) {
  return equipamentos
    .map(formatEquipamentoForDisplay)
    .filter(Boolean)
    .join("\n");
}
...
async function carregarChamados(options = {}) {
  ...
  if (useCache && !state.chamados.length) {
    hidratarListaComCache("painel");
  }

  if (!silent) setLoading(reloadBtn, true, "Atualizando...");

  try {
    const response = await apiGet("listChamados");
    state.chamados = Array.isArray(response.data) ? response.data : [];
    salvarChamadosNoCache(state.chamados);
    aplicarFiltros();
    ...
    console.error(error);
    if (!silent) toast(error.message || "Erro ao carregar chamados.", "error");
    if (!state.chamados.length) {
      state.chamados = [];
      aplicarFiltros();
    }
  } finally {
    if (!silent) setLoading(reloadBtn, false, "🔄 Atualizar");
  }
}
...
function atualizarChamadoNoEstado(chamadoAtualizado) {
  ...
  const upsertChamado = (lista) => {
    ...
    lista.unshift(chamadoAtualizado);
  };

  upsertChamado(state.chamados);
  upsertChamado(state.adminChamados);
...
function initOSModal() {
  ...
  window.addEventListener("focus", restoreDocumentTitleSoonAfterPrint);
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) restoreDocumentTitleSoonAfterPrint();
  });
}
...
async function imprimirOS() {
  ...
  const nomeArquivoOS = buildOSPrintFilename();
  setDocumentTitleForPrint(nomeArquivoOS);

  // No Android/Chrome, a tela "Salvar em PDF" costuma usar o título atual da página.
  // Este pequeno atraso dá tempo para o navegador atualizar o título antes de abrir a impressão.
  await wait(650);
  ...
  // Mantém o título por mais tempo porque alguns Androids leem o nome depois que o diálogo já abriu.
  scheduleDocumentTitleRestore(90000);
}

function setDocumentTitleForPrint(nomeArquivo) {
  ...
  if (state.documentTitleRestoreTimer) {
    window.clearTimeout(state.documentTitleRestoreTimer);
    state.documentTitleRestoreTimer = null;
  }

  document.title = nomeArquivo;

  const titleTag = document.querySelector("title");
  if (titleTag) titleTag.textContent = nomeArquivo;
}
...
function restoreDocumentTitleSoonAfterPrint() {
  scheduleDocumentTitleRestore(8000);
}

function scheduleDocumentTitleRestore(delay = 90000) {
  if (!state.documentTitleBeforePrint) return;

  if (state.documentTitleRestoreTimer) {
    window.clearTimeout(state.documentTitleRestoreTimer);
  }

  state.documentTitleRestoreTimer = window.setTimeout(() => {
    restoreDocumentTitleNow();
  }, delay);
}

function restoreDocumentTitleNow() {
  if (!state.documentTitleBeforePrint) return;

  document.title = state.documentTitleBeforePrint;

  const titleTag = document.querySelector("title");
  if (titleTag) titleTag.textContent = state.documentTitleBeforePrint;
...
function abrirOS(chamado) {
  ...
  setValue("#osCnpj", maskCNPJ(chamado.cnpj));
  ...
  setValue("#osTelefone", maskPhone(chamado.telefone));
...
function renderEquipamentos(chamado) {
  ...
  const equipamentos = parseEquipamentosFromChamado(chamado);
  (equipamentos.length ? equipamentos : [{}]).forEach((equipamento) => addEquipamentoRow(equipamento));
}

function addEquipamentoRow(value = {}) {
  const container = $("#equipamentosContainer");
  if (!container) return;
  const row = createEquipamentoRow(container, value, "os-equipamento-row");
  container.appendChild(row);
}
...
async function salvarOS(event) {
  ...
  try {
    const equipamentos = coletarOSEquipamentos();
    ...
    preparePrintValues();
    salvarChamadosNoCache(state.chamados.length ? state.chamados : state.adminChamados);
...
function demoApi(action, data) {
  ...
  if (action === "createChamado") {
    ...
    const novo = {
      ...
      modeloEquipamento: data.modeloEquipamento || formatEquipamentosForDisplay(data.equipamentos || []),
      equipamentosJson: JSON.stringify(data.equipamentos || parseEquipamentosFromChamado(data)),
      fotosJson: "[]"
    };
  ...
  if (action === "updateOS") {
    ...
    chamados[index] = {
      ...
      modeloEquipamento: formatEquipamentosForDisplay(data.equipamentos || []),
...
function validateCNPJField() {
  ...
  const valid = hasCNPJRequiredLength(input.value);
  help.textContent = valid ? "CNPJ com quantidade correta." : "Informe CNPJ com 14 dígitos.";
...
function validatePhoneField() {
  ...
  const valid = hasPhoneRequiredLength(input.value);
  help.textContent = valid ? "Telefone com quantidade correta." : "Informe DDD + telefone com 10 ou 11 dígitos.";
  help.className = `field-help ${valid ? "ok" : "error"}`;
  return valid;
}

function hasCNPJRequiredLength(value) {
  return onlyDigits(value).length === 14;
}

function hasPhoneRequiredLength(value) {
