...
const CACHE_KEY_PREFIX = 'MEDPLUS_CHAMADOS_CACHE_V4_';
const COUNTER_PROPERTY_PREFIX = 'MEDPLUS_CHAMADO_COUNTER_';
const DRIVE_FOLDER_ID_PROPERTY_PREFIX = 'MEDPLUS_DRIVE_FOLDER_ID_';
...
function CRIAR_ESTRUTURA() {
  ...
  sincronizarContadorChamados_(ss, sheet);
  invalidateChamadosCache_(ss);
...
function updateChamado_(ss, data) {
  ...
  const updatedObject = updateRowObject_(sheet, found.row, current, updates);
  invalidateChamadosCache_(ss);
  log_(ss, 'UPDATE_CHAMADO', numeroChamado, 'Chamado editado pela área administrativa da abertura.');

  return rowToChamado_(updatedObject);
}
...
function updateOS_(ss, data) {
  ...
  const current = rowArrayToObject_(found.values);
  const temFotosNovas = Array.isArray(data.fotos) && data.fotos.length;
  const temAssinaturaClienteNova = data.assinaturaCliente && data.assinaturaCliente.base64;
  const temAssinaturaTecnicoNova = data.assinaturaTecnico && data.assinaturaTecnico.base64;
  const folder = (temFotosNovas || temAssinaturaClienteNova || temAssinaturaTecnicoNova) ? createDriveFolder_(ss) : null;
...
function listChamados_(ss) {
  const cached = getChamadosFromCache_(ss);
...
function generateUniqueNumber_(sheet) {
  ...
  try {
    ...
    const key = getCounterPropertyKey_(ss);
    let current = Number(props.getProperty(key) || '0');

    if (!current || current < 0) {
      current = getHighestChamadoNumber_(sheet);
    }

    current += 1;
    props.setProperty(key, String(current));

    return 'MED-' + String(current).padStart(4, '0');
  } finally {
    try {
      lock.releaseLock();
    } catch (_) {}
...
function getCounterPropertyKey_(ss) {
  return COUNTER_PROPERTY_PREFIX + getSpreadsheetId_(ss);
}

function getHighestChamadoNumber_(sheet) {
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return 0;

  const values = sheet.getRange(2, 1, lastRow - 1, 1).getValues();
  return values.reduce((max, row) => {
    const match = String(row[0] || '').match(/MED-(\d+)/i);
    const num = match ? Number(match[1]) : 0;
    return Number.isFinite(num) && num > max ? num : max;
  }, 0);
}

function sincronizarContadorChamados_(ss, sheet) {
  try {
    const props = PropertiesService.getScriptProperties();
    const key = getCounterPropertyKey_(ss);
    const current = Number(props.getProperty(key) || '0');
    const highest = getHighestChamadoNumber_(sheet);
    if (highest > current) props.setProperty(key, String(highest));
  } catch (_) {}
}

function updateRowObject_(sheet, rowNumber, currentObject, updates) {
  const rowValues = HEADERS.map((header) => {
    if (Object.prototype.hasOwnProperty.call(updates, header)) return updates[header];
    return currentObject[header] || '';
  });

  sheet.getRange(rowNumber, 1, 1, HEADERS.length).setValues([rowValues]);
  return rowArrayToObject_(rowValues);
}

function getSpreadsheetId_(ss) {
  try {
    return ss && ss.getId ? ss.getId() : DEFAULT_SPREADSHEET_ID;
  } catch (_) {
    return DEFAULT_SPREADSHEET_ID;
  }
}

function getChamadosCacheKey_(ss) {
  return CACHE_KEY_PREFIX + getSpreadsheetId_(ss);
}

function getChamadosFromCache_(ss) {
  try {
    const cache = CacheService.getScriptCache();
    const raw = cache.get(getChamadosCacheKey_(ss));
    if (!raw) return null;
...
function saveChamadosToCache_(ss, data) {
  try {
    if (!Array.isArray(data)) return;
    const payload = JSON.stringify(data);
    if (payload.length > 90000) return;
    CacheService.getScriptCache().put(getChamadosCacheKey_(ss), payload, CACHE_TTL_SECONDS);
  } catch (_) {}
}

function invalidateChamadosCache_(ss) {
  try {
    CacheService.getScriptCache().remove(getChamadosCacheKey_(ss));
  } catch (_) {}
}
...
function createDriveFolder_(ss) {
  const props = PropertiesService.getScriptProperties();
  const propKey = DRIVE_FOLDER_ID_PROPERTY_PREFIX + getSpreadsheetId_(ss);
  const savedFolderId = props.getProperty(propKey);
  ...
  if (parentFolder) {
    ...
    if (folders.hasNext()) {
      ...
      props.setProperty(propKey, existingFolder.getId());
